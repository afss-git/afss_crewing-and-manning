...
function ProfileCreatePage() {
  ...
  // Load existing profile data on component mount (in case user is editing)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Submitting profile data with accessToken");
      ...
      if (response.ok) {
        ...
        // Mark that profile was just created to prevent redirect loop
        if (typeof window !== "undefined") {
          sessionStorage.setItem("shipowner_profile_created", "true");
        }
        // Redirect to contract type selection
        ...
        console.error(
          "Error response:",
          errorBody ?? (rawText ? rawText : { status: response.status, statusText: response.statusText }),
        );
        ...
        alert(`Failed to create profile: ${errMsg}`);
      }
    } catch (error) {
        ...
        <div className="w-full max-w-[1024px] space-y-10">
          ...
          <div className="flex flex-col gap-3 text-center md:text-left">
            ...
            <p className="text-text-secondary text-lg max-w-2xl font-normal leading-relaxed">
              Provide your company information to get started managing your crew
              operations.
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full px-4 py-3 rounded-lg border border-[#d1d8e6] dark:border-slate-600 bg-[#f8f9fb] dark:bg-slate-800 text-text-main dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
                      placeholder="e.g., Fleet Manager, Director"
                      required
                    />
                  </div>
                  <div>
              ...
              <div className="bg-surface-light dark:bg-surface-dark p-8 rounded-xl border border-[#e8ebf3] dark:border-gray-800">
                ...
                <p className="text-text-secondary text-sm mb-6">
                  Upload relevant company documents such as registration
                  certificates, maritime authority licenses, or compliance
                  documentation.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[1, 2, 3, 4].map((docNum) => (
                    <div key={docNum}>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-4 py-3 rounded-lg border border-[#d1d8e6] dark:border-slate-600 bg-[#f8f9fb] dark:bg-slate-800 text-text-main dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-primary-light"
                          accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                        />
                      </div>
                      {documents[
                        `document_${docNum}` as keyof typeof documents
                      ] && (
              ...
              <div className="flex gap-4 justify-center md:justify-start">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="px-8 py-3 rounded-lg bg-primary hover:bg-primary-light text-white font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading
                    ? "Creating Profile..."
