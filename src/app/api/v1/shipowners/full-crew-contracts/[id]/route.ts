...
export async function GET(
  ...
  try {
    ...
    console.log(`🔗 Fetching full-crew-contract ${id}...`);

    const backendUrl = `https://crew-manning.onrender.com/api/v1/shipowners/full-crew-contracts/${id}`;
    ...
    console.log(`✅ Successfully fetched full-crew-contract ${id}`);

    return NextResponse.json(responseData, { status: 200 });
  } catch (error) {
    console.error("Get full-crew-contract error:", error);
    return NextResponse.json(
      { detail: "Failed to fetch full-crew-contract", error: String(error) },
      { status: 500 }
    );
  }
}
